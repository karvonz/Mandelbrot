...
#define nMemoryRead(A)     (*(volatile unsigned int*)(A))
#define nMemoryWrite(A,V) *(volatile unsigned int*)(A)=(V)

int nputchar(int value)
{
   while((nMemoryRead(IRQ_STATUS) & IRQ_UART_WRITE_AVAILABLE) == 0);
   nMemoryWrite(UART_WRITE, value);
	//int i;
	//for(i=0; i<65536; i++){ 	asm volatile("nop" : : ); }
   return 0;
}

int nputs(const char *string)
{
   while(*string)
   {
      //if(*string == '\n')
