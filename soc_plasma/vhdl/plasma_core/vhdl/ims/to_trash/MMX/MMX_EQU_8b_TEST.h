...
//
////////////////////////////////////////////////////////////////
#define MMX_EQU_8b_TEST_LENGTH	10

#define OP1(a) ((a & 0x000000FF))
#define OP2(a) ((a & 0x0000FF00)>> 8)
#define OP3(a) ((a & 0x00FF0000)>>16)
#define OP4(a) ((a & 0xFF000000)>>24)
int MMX_EQU_8b_AUTOTEST(){
	int i, j;
	printf("LAUNCHING THE (MMX_EQU) RESOURCE AUTOTEST...\n");
	for(j=0; j<MMX_EQU_8b_TEST_LENGTH; j++){
		int op1 = ((rand() % 65536) << 16) & (rand() % 65536);
		int op2 = ((rand() % 65536) << 16) & (rand() % 65536);
		int op3 = instr_v8equ(op1, op2);
		int op4 = prog_v8equ (op1, op2);
		if( op3 != op4 ){
			printf("- TEST OF THE (MMX_EQU) RESOURCE => FAILED !\n");
	...
	printf("- TEST OF THE MMX_EQU_8b RESOURCE => OK\n");
	return 1;
}
