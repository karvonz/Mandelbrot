...
for l in tbl_file:
	...
	if l[0]=="#" or len(a)==0:
		continue
	assert int(a[0]) == slv_idx
	slv_idx = slv_idx + 1
	addr_bits = BitArray(int = int(a[2], 16), length = 32)
	mask_bits = BitArray(int = (2**int(a[3], 10))-1, length = 32)
...
for l in tmpl_file:
	if l.rstrip()!="--ADDR_TABLE_HERE":
		...
		for s in slaves:
			...
			out_file.write("\t\tif    " if int(s[0])==0 else "\t\telsif ")
			out_file.write("std_match(addr, \"" + cf_str + "\") then\n")
			out_file.write("\t\t\tsel := " + s[0] + "; -- " + s[1] + " / base " + s[2].hex + " / mask " + s[3].hex + "\n")
